...
def bts(num_answer_options, votes):
    ...
    for vote in votes:
        ...
        for answer in vote.prediction:
            total_prediction_percentage += vote.prediction[answer]
        if total_prediction_percentage != 100:
    ...
    for i in range(len(total_votes)):
        ...
        print(F"Endorsement frequencies vor answer {i + 1}: {round(endorsement_frequencies[i] * 100, 2)}%")
    ...
    for vote in votes:
        for answer in vote.prediction:
            if (vote.prediction[answer] != 0):
                predicted_frequencies[answer - 1] += math.log10(vote.prediction[answer])
                arithmetic_avg_predicted_frequencies[answer - 1] += vote.prediction[answer]
...
prediction6 = { 1: 10, 2: 10, 3: 10, 4 : 70 }
